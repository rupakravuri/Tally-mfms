...
import { Building2, Server, Database, AlertCircle, CheckCircle, Loader2, FileText, Package, Settings, RefreshCw } from 'lucide-react';
...
const TallyFieldExtractor: React.FC<TallyFieldExtractorProps> = ({ 
  ...
  const [currentView, setCurrentView] = useState<'connection' | 'company' | 'fields' | 'sync' | 'mongoconfig' | 'fieldmapping'>('connection');
  ...
  const selectCompany = (companyName: string) => {
    ...
    setCurrentView('sync'); // Navigate to sync instead of fields
  };
  ...
  const renderSyncView = () => (
    <div className="max-w-7xl mx-auto">
      ...
      <SyncInterface 
        ...
        onConfigureFields={() => setCurrentView('fieldmapping')}
        onConfigureMongo={() => setCurrentView('mongoconfig')}
      />
    </div>
  );

  const renderMongoConfigView = () => (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <button
      ...
      <MongoConfig 
        onConfigured={() => setCurrentView('sync')}
        onClose={() => setCurrentView('sync')}
      />
    </div>
  );

  const renderFieldMappingView = () => (
    <div className="max-w-7xl mx-auto">
      <div className="mb-6">
        <button
          onClick={() => setCurrentView('sync')}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-4">
      ...
      {currentView === 'mongoconfig' && renderMongoConfigView()}
      {currentView === 'fieldmapping' && renderFieldMappingView()}
    </div>
  );
};
